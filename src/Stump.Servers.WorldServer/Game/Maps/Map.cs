...
namespace Stump.Server.WorldServer.Game.Maps
{
	public class Map : WorldObjectsContext, ICharacterContainer
	{
		...
		public void UpdateFightPlacements(byte nbFighters = 0)
		{
			...
			if (Record.BlueFightCells.Length == 0 || Record.RedFightCells.Length == 0)
			{
				...
				else if (Cells.Count(x => x.Walkable && !x.FarmCell && !x.NonWalkableDuringFight) > 16)
				{
					...
					var zone = new Zone(shape, (byte)rand.Next(16, 20), (byte)rand.Next(1, 3));
					var interacCells = GetInteractiveObjects().Select(y => y.Cell);
					var fightcells = zone.GetCells(this.GetRandomWalkableCell(x => !x.FarmCell && !x.NonWalkableDuringFight), this).Where(x => x.Walkable && !x.NonWalkableDuringRP && !interacCells.Contains(x) && !x.NonWalkableDuringFight);
					if (fightcells.Count() < 16)
					{
						...
						while (cells.Count != 16)
						{
							...
							if (!cells.Any(x => x.Id == tata))
								if (Cells[tata].Walkable && !Cells[tata].FarmCell && !Cells[tata].NonWalkableDuringFight && !Cells[tata].NonWalkableDuringRP && !interacCells.Contains(Cells[tata]))
					...
					{
						int half = fightcells.Count() / 2;
						int secondhalf = fightcells.Count() - half;
						m_bluePlacement = new Cell[half];
						m_redPlacement = new Cell[secondhalf];
						Array.Copy(fightcells.ToArray(), 0, m_bluePlacement, 0, half);
						Array.Copy(fightcells.ToArray(), half, m_redPlacement, 0, secondhalf);
		...
		{
			get { return Record.IsInstantiated; }
		}
		...
		{
			if (m_bluePlacement == null || m_redPlacement == null)
				return;
			if (!CanSpawnMonsters())
				return;
		...
		public MonsterGroup GenerateRandomMonsterGroup(int length)
		{
			...
			var group = new MonsterGroup(GetNextContextualId(), new ObjectPosition(this, GetRandomFreeCellAnyTimeWalkable(), GetRandomDirection()));
		...
		public MonsterGrade NaniMonster;
		public MonsterGroup SpawnMonsterGroup(MonsterGrade monster, ObjectPosition position, bool Nani = false)
		{
			if (position.Map != this)
		...
		public Cell[] BlueFightCells
		{
			get { return m_bluePlacement; }
			set { m_bluePlacement = value; }
		}
		public Cell[] RedFightCells
		{
			get { return m_redPlacement; }
			set { m_redPlacement = value; }
		}
		...
		public void Enter(RolePlayActor actor)
		{
			...
			if (actor is Character)
			{
				...
				if (IsInstantiated)
					(actor as Character).SendServerMessage("Cette carte est soumise à une instance temporelle, Xélor vous restreind le fait de pouvoir intéragir avec d'autres joueurs !");
			}
			Refresh(actor);
		}
		...
		void OnEnter(RolePlayActor actor)
		{
			...
			if (character != null)
			{
				if (character.UserGroup.Id < 2 && (Id == 88212759 || Id == 84675329)) character.TogglePvPMode(true);
		...
		public void ForEach(Action<Character> action)
		{
			...
			foreach (var character in Characters)
			{
				...
				{
					logger.ErrorException("Error while executing action on character ", ex);
				}
			}
		}
		...
		public bool IsCellFreeActor(FightActor actorOfFight, short cell)
		{
			return actorOfFight.Fight.Fighters.All(x => x.Position.Point.CellId != cell);
		}
		...
		public Cell GetFirstFreeCellInDirectionNear(FightActor actor, Cell goal)
		{
			var positiongoal = new MapPoint(goal);
			var cells = positiongoal.GetCellsInLine(actor.Position.Point).FirstOrDefault(x => actor.Map.IsCellWalkable(actor.Map.GetCell(x.CellId)) && x.CellId != positiongoal.CellId && actor.Fight.Fighters.All(y => y.Position.Point.CellId != x.CellId));

			if (cells != null)
				return GetCell(cells.CellId);
		...
		public void ActivateInteractiveObjectForASpecificPlayer(Character character, int interactiveId)
		{
			var interactive = GetInteractiveObject(interactiveId);

			if (interactive == null)
				return;

			if (!interactive.CharacterCanSee.Contains(character))
				interactive.CharacterCanSee.Add(character);

			interactive.SetInteractiveState(InteractiveStateEnum.STATE_ACTIVATED);
			InteractiveHandler.SendInteractiveElementUpdatedMessage(character.Client, character, interactive);
		}

		public void DisableActivateStateInteractiveObjectForASpecificPlayer(Character character, int interactiveId)
		{
			var interactive = GetInteractiveObject(interactiveId);
		...
		public Cell GetRandomFreeCellAnyTimeWalkable(bool actorFree = false, bool nearMiddle = false)
		{
			var rand = new AsyncRandom();
			var selection = m_freeCells.Where(x => !x.NonWalkableDuringFight && !x.NonWalkableDuringRP && x.Walkable);

			if (!actorFree)
				return selection.ElementAt(rand.Next(0, selection.Count()));

			var excludedCells = GetActors<RolePlayActor>().Select(entry => entry.Cell.Id);
			var cells = selection.Where(entry => !excludedCells.Contains(entry.Id)).ToArray();
		...
		{
			var interractives = m_interactives.Where(entry => entry.Value.CanBeSee(character)).Select(entry => entry.Value.GetInteractiveElementWithBonus(character)).ToArray();

			try
			{
				if (HavenBagManager.Instance.IsHavenBagMap(Record.Id))
				{
					var mcifihm = new MapComplementaryInformationsDataInHavenBagMessage(
					(ushort)SubArea.Id,
					Id,
					new HouseInformations[0],
					m_actors.Where(entry => entry.CanBeSee(character)).Select(entry => entry.GetGameContextActorInformations(character) as GameRolePlayActorInformations).ToArray(),
					m_interactives.Where(entry => entry.Value.CanBeSee(character)).Select(entry => entry.Value.GetInteractiveElementWithBonus(character)).ToArray(),
					m_interactives.Where(entry => entry.Value.CanBeSee(character)).Where(x => x.Value.Animated).Select(entry => entry.Value.GetStatedElement()).ToArray(),
					GetMapObstacles().ToArray(),
					m_fights.Where(entry =>
					entry.BladesVisible)
					.Select(entry =>
					entry.GetFightCommonInformations()).ToArray(),
					false,
					new FightStartingPositions(GetBlueFightPlacement()
					.Select(x =>
					(ushort)x.Id).ToArray(),
					GetRedFightPlacement().Select(x => (ushort)x.Id).ToArray()),
					HavenBagManager.Instance.GetHavenBagOwner(character).GetCharacterMinimalInformations(),
					HavenBagManager.Instance.GetHavenBagTheme(character),
					HavenBagManager.Instance.GetHavenBagRoomId(character),
					HavenBagManager.Instance.GetHavenBagMaxRoom(character));
