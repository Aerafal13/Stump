...
namespace Stump.Server.WorldServer.Game.Actors.RolePlay.Characters
{
	public sealed class Character : Humanoid, IStatsOwner, IInventoryOwner, ICommandsUser
	{
		...
		void OnLoggedIn()
		{
			...
			{
				var item = Inventory.TryGetItem(ItemManager.Instance.TryGetTemplate(15236));
				if (!WorldAccount.Vip && !this.Inventory.HasItem(ItemManager.Instance.TryGetTemplate(15236)))
				{
					item = this.Inventory.AddItem(ItemManager.Instance.TryGetTemplate(15236), 1);
				...
				if (item != null)
					this.Shortcuts.AddItemShortcut(8, item);

				item = Inventory.TryGetItem(ItemManager.Instance.TryGetTemplate(16147));
				if (WorldAccount.Vip && !this.Inventory.HasItem(ItemManager.Instance.TryGetTemplate(16147)))
				{
					item = this.Inventory.AddItem(ItemManager.Instance.TryGetTemplate(16147), 1);
					item.Effects.Add(new EffectInteger(EffectsEnum.Effect_NonExchangeable_981, 1));
					item.Invalidate();
					this.Inventory.RefreshItem(item);
				}
				...
				if (item != null)
					this.Shortcuts.AddItemShortcut(19, item);
				...
				if (item != null)
					this.Shortcuts.AddItemShortcut(9, item);
